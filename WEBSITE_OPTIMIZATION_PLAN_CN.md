...
优秀的 Facelift / Deep Plane Facelift 页面，不只是内容完整，更重要的是每一屏都有独立的视觉吸引力。页面应该像高端医美画册或杂志跨页：大图、留白、短标题、克制正文、明确按钮，形成“看一屏就愿意继续往下滑”的节奏。

后续改造每个栏目页时，优先遵守以下原则：

1. 每一屏尽量有一个清晰主题，不把太多信息挤在一起
2. 重要服务页使用大图、视频或半屏图片配文字，避免整页都是文字卡片
3. 文案保持短、准、高级，正文不要堆满，要让用户愿意读
4. 图片和文字采用左右分栏、上下交错、暗色覆层等方式制造高级感
5. 首屏必须有强钩子：用户痛点、自然效果、医生权威、咨询按钮
6. 第二屏到第三屏尽快建立信任：医生、技术、适合人群、真实案例
7. 每隔 1-2 屏放一次轻 CTA，例如 WhatsApp、提交照片评估、在线咨询
8. FAQ、恢复期、风险、价格等内容也要设计成模块，不要像普通说明书
9. 移动端每屏更要短，图片、标题、按钮必须一眼看懂
10. 参考欧美站的 SEO 结构，参考亚洲医美站的视觉情绪，再形成 Dr. Xiao 9D 自己的风格

重点学习对象：

- 欧美 Facelift / Deep Plane 页面：学习医学解释、SEO 长页面、FAQ、医生权威布局
- 韩国、泰国、华人医美页面：学习大图视觉、咨询引导、案例氛围、亚洲脸适配表达
- 高端医生个人品牌页面：学习极简排版、留白、医生照片和信任信号渗透

后续优先改造页面：

1. `/procedures/9d-facelift`
2. `/procedures/9d-deep-plane-facelift`
...
每天只选 1-2 个最值得改的小模块，先做视觉结构和转化路径，再逐步补 SEO 内容、案例、FAQ、结构化数据和 CMS 接入。所有重要页面改造前，先分析竞品画面结构，再决定我们如何模仿并超越。
